...
const GlobalNav: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-sm border-b border-border px-3 py-2 flex items-center gap-3">
      {/* 回家按钮 */}
      <button onClick={handleHome} className="touch-target rounded-2xl bg-primary/10 hover:bg-primary/20 active:scale-95 transition-all px-3 py-1 text-2xl" aria-label="回到首页">
        🏠
      </button>
      ...
      {/* 头像/退出 */}
      <button onClick={handleLogout} className="touch-target text-3xl" aria-label="切换档案">
